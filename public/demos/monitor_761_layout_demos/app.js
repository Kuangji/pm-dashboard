...
const rows = [
  ['Summer Routine 2026 | skincare refresh', 'Lena Beauty Lab', 'May Launch', 'YT', '128.4K', '+18.2K', '4.8%', '19/60天', 31, '自动追踪 · Rule A', '监控中', 'green'],
  ['Unboxing new mini camera for travel creators', 'Mika Studio', 'Beauty Review', 'TT', '86.1K', '+9.7K', '6.2%', '42/60天', 70, '手动添加', '即将结束', 'orange'],
  ['GRWM: soft glam for weekend brunch', 'Ava Daily', 'TikTok Spring', 'IG', '54.3K', '+1.1K', '--', '58/60天', 96, '自动追踪 · Rule C', '评论不足', 'red'],
  ['Honest review after 30 days of usage', 'TechSkin Review', 'Camera Trial', 'X', '23.9K', '+0', '1.3%', '12/30天', 40, '手动添加', '短链缺失', 'blue'],
  ['Creator haul recap with manual budget', 'Nora Finds', 'Holiday Seeding', 'YT', '12.6K', '+320', '2.9%', '60/60天', 100, '手动添加', '已结束', 'gray'],
  ['Short-form paid review without budget input', 'Kai Shorts', 'May Launch', 'TT', '9.4K', '+860', '5.1%', '8/30天', 26, '自动追踪 · Rule B', '预算未填', 'red']
].map(([title, creator, project, platform, views, inc, er, progress, pct, source, status, statusClass]) => ({
  title,
  creator,
  project,
  platform,
...
const projectOptions = [
  ['May Creator Launch', 'YouTube / TikTok', 46, true],
  ['Beauty Review', 'Instagram / TikTok', 38, true],
  ['TikTok Spring Test', 'TikTok', 21, true],
  ['Camera Trial', 'YouTube / X', 12, false],
  ['Holiday Seeding', 'Instagram', 11, false]
]
...
function scopeMeta(type = initialScope) {
  if (type === 'single') {
    ...
    return {
      title: projectName,
      desc: '单项目范围 · Yuki Chen · 协作者 4 · 创建于 2026-04-18',
      content: '当前内容 46',
      platform: '覆盖平台 3',
      issue: '需关注 7',
      mode: '单项目范围'
      ...
      <div class="workbench-title-block">
        <div class="scope-headline" data-scope-bar>
          <div>
            ...
            <h2><span data-scope-title>${meta.title}</span><span class="scope-edit-icon" data-open-drawer="scope-config" role="button" title="编辑项目范围" aria-label="编辑项目范围">✎</span></h2>
          ...
          <div class="scope-summary">
            ...
            <button class="btn btn-primary scope-add-monitor">添加监控</button>
          </div>
        </div>
      </div>
    </div>
...
function projectHeader(page) {
  return `
    ...
    <div class="filter-row">
      <span class="select">全部 (128)</span><span class="select">地区</span><span class="select">视频标签</span><span class="select">额外标签1</span><span class="select">额外标签2</span><span class="select">负责人</span><span class="select">真实创作者</span><span class="select">监控时长</span><span class="select">观看量</span><span class="select">视频类型</span><span class="select">来源</span><span class="select date-select">发布日期</span>
    </div>`
}
    ...
    <aside class="config-drawer scope-drawer" data-config-drawer="scope-config" data-scope-editor hidden>
      ...
      <div class="scope-editor-body">
        ...
        <div class="scope-filter-tools">
          <span class="select">卡片状态：进行中</span>
          <span class="select">负责人：全部</span>
          <span class="select">平台：全部</span>
          <div class="search">搜索项目名称</div>
        </div>
        <div class="scope-projects scope-projects-drawer">
          ${projectOptions
            .map(([name, platforms, count, checked], index) => {
              ...
              return `
                <label class="scope-project ${isChecked ? 'checked' : ''}" data-scope-project data-project-id="${projectId}" data-content-count="${count}">
                  ...
                  <span><strong>${name}</strong><em>${platforms} · ${count} 条内容</em></span>
                </label>`
            })
            .join('')}
        </div>
  ...
  return `
    <div class="global-filter-sticky">
      <div class="filter-row workbench-filter-row">
        ...
        <span class="select">全部 (128)</span>
        <span class="select">地区</span>
        <span class="select">视频标签</span>
        <span class="select">额外标签1</span>
        <span class="select">额外标签2</span>
        <span class="select">负责人</span>
        <span class="select">真实创作者</span>
        <span class="select">监控时长</span>
        <span class="select">观看量</span>
        <span class="select">视频类型</span>
        <span class="select">来源</span>
        <span class="select date-select">发布日期</span>
      </div>
    </div>`
}
...
const diagnosticMetrics = [
  ['观看增量', '428.6K', '可观测增长', '按当前数据观察区间计算，表示所选内容集合在观察区间内的观看增长。'],
  ['Engagement 增量', '41.1K', '点赞 + 评论 + 分享', '点赞、评论、分享在观察区间内的增量合计。'],
  ['活跃内容', '42', '有观看或互动变化', '观察区间内观看、点赞、评论或分享发生变化的内容数。'],
  ['需关注内容', '7', '低增长 / 配置缺失', '包含低增长、数据不可解释、评论样本不足、短链或预算配置缺失等内容。']
]
...
const rawMetrics = [
  ...
  ['自定义数据', '12.3K', '用户回填或外部导入数据，默认展示当前值，不强行套用观察区间。', false]
]
...
const drilldownViews = {
  ...
  active: {
    ...
    rows: [
      ...
      ['Mini camera unboxing', '互动变化', '手动添加', '即将结束'],
  ...
  rank: {
    ...
    process: '可切换榜单指标、打开单内容抽屉，或把当前排序应用到列表。',
    quickActions: ['切换榜单指标', '打开单内容抽屉', '应用排序到列表'],
    columns: ['内容', '观看', '点赞', 'ER'],
    rows: [
...
function metricCards(compact = false) {
  const overview = diagnosticMetrics.map(([label, value, note]) => [label, value, note])
  if (compact) {
  ...
  return `
    <section class="diagnostic-summary">
      <div class="section-head"><div><h2 class="section-title">观察区间诊断摘要</h2><span class="section-sub">按数据观察周期汇总当前内容集合。</span></div></div>
      <div class="metrics diagnostic-metrics">${diagnosticMetrics.map(([label, value, tag, note], index) => `<div class="metric diagnostic-card ${index === 3 ? 'attention-card' : ''}">${metricHelp(note)}<span class="metric-tag">${tag}</span><div class="metric-label">${label}</div><div class="metric-value">${value}</div></div>`).join('')}</div>
    </section>`
}
  ...
  return `
    <div class="observation-control ${compact ? 'compact-observation-control' : ''}" data-observation-control>
      ...
      <div class="observation-popover" data-observation-popover hidden>
        ...
        <span class="observation-note">影响看板、原始数据增量、榜单和列表增量列</span>
      </div>
    </div>`
}
...
function rankList(title = '内容级变化榜') {
  ...
  return `<section class="panel rank-panel"><div class="section-head"><div><h2 class="section-title">${title}</h2><span class="section-sub">榜单指标可切换；模块动作应用为列表排序，不改变看板统计口径。</span></div><button class="btn btn-ghost" data-apply-list-filter="rank">应用到列表</button></div><div class="rank-metric-tabs"><span class="active" data-apply-list-filter="rank">观看增量</span><span data-apply-list-filter="engagement">Engagement</span><span data-apply-list-filter="likes">点赞</span><span data-apply-list-filter="comments">评论</span><span data-apply-list-filter="shares">分享</span><span data-apply-list-filter="er">ER</span></div><div class="rank-table"><div class="rank-table-head"><span>内容</span><span>观看</span><span>点赞</span><span>评论</span><span>分享</span><span>ER</span></div>${rows.map((r, i) => `<button class="rank-table-row" data-open-drawer="content-detail"><div><span class="rank-index">${i + 1}</span><div><div class="rank-title">${r[0]}</div><div class="rank-meta">${r[6]}</div></div></div><strong>${r[1]}</strong><span>${r[2]}</span><span>${r[3]}</span><span>${r[4]}</span><span>${r[5]}</span></button>`).join('')}</div></section>`
}

function autoTrackNewPanel() {
  const items = [
    ['Rule A', 'Summer Routine 2026', 'Lena Beauty Lab', 'YT', '12 分钟前', '监控中'],
    ['Rule B', 'Short-form paid review', 'Kai Shorts', 'TT', '38 分钟前', '预算未填'],
    ['Rule C', 'GRWM weekend brunch', 'Ava Daily', 'IG', '52 分钟前', '评论不足']
  ]
  return `<section class="panel auto-track-panel"><div class="section-head"><div><h2 class="section-title">近期自动追踪新增</h2><span class="section-sub">按监控任务入库时间展示，不等同于发布日期或数据观察周期。</span></div><button class="btn btn-ghost" data-apply-list-filter="auto-track">查看全部</button></div><div class="auto-track-summary"><span>最近 1 小时新增 <strong>3</strong> 条</span><span>命中规则 <strong>3</strong> 个</span><span>通知已汇总发送</span></div><div class="auto-track-list">${items.map(([rule, title, creator, platform, time, status]) => `<button class="auto-track-item" data-open-drawer="content-detail"><span class="rule-pill">${rule}</span><div><strong>${title}</strong><em>${creator} · ${platform} · ${time}</em></div><b>${status}</b></button>`).join('')}</div></section>`
}

function contributionPanel() {
  return `<section class="panel contribution-panel"><div class="section-head"><h2 class="section-title">达人 / 频道贡献</h2><span class="section-sub">项目范围内聚合，不跨平台合并</span></div>${[['Lena Beauty Lab', '46%'], ['Mika Studio', '24%'], ['Ava Daily', '13%'], ['TechSkin Review', '8%']].map(n => `<button class="progress-row" data-apply-list-filter="channel"><div class="progress-name">${n[0]}</div><div class="progress-track"><div class="progress-fill" style="width:${n[1]}"></div></div><div class="progress-value">${n[1]}</div></button>`).join('')}</section>`
}
  ...
  return `
    <aside class="config-drawer drilldown-drawer task-detail-drawer" data-config-drawer="content-detail" hidden>
      ...
      <div class="drilldown-body">
        ...
        <section class="detail-section">
          <div class="detail-section-head"><h3>视频/贴文数据趋势</h3><div class="detail-tools"><span class="view-switch"><span class="active">⌁</span><span>☷</span></span><span class="select">2026-03-13 ~ 2026-05-12</span></div></div>
...
function configDrawers() {
  ...
  const metricItems = ['观看量', '点赞数', '评论数', '分享数', 'Engagement 总量', '互动率', 'Nox 短链点击数', '预算消耗', 'CPM', 'CPV', 'CTR', '自定义数据'].map((name, index) => [name, index < 7, requiredMetricNames.includes(name)])
  const columnItems = [
    ['内容', true, true],
...
function table() {
  ...
  return `
    <div class="table-wrap"><table>
      <thead><tr><th class="select-col"><input type="checkbox" aria-label="选择全部" data-select-all-rows></th><th style="width:30%">内容</th>${isCrossProject ? '<th style="width:14%">所属项目</th>' : ''}<th><button class="sort-head" data-apply-list-filter="views">观看量</button></th><th><button class="sort-head active" data-apply-list-filter="rank">观察区间增量</button></th><th><button class="sort-head" data-apply-list-filter="er">互动率</button></th><th>进度</th><th style="width:16%">来源</th><th>状态</th><th class="table-config-head"><button class="icon-btn" data-open-drawer="column-config" title="配置表头" aria-label="配置表头">⚙</button></th></tr></thead>
      <tbody>${rows.map((row, index) => `<tr data-content-row data-open-task-detail><td class="select-col"><input type="checkbox" aria-label="选择 ${row.title}" data-content-check></td><td><div class="content-cell"><div class="thumb"><span class="platform">${row.platform}</span></div><div class="nowrap"><div class="cell-title">${row.title}</div><div class="cell-sub">${row.creator}</div></div></div></td>${isCrossProject ? `<td><span class="chip blue">${row.project}</span></td>` : ''}<td>${row.views}</td><td><span class="${row.inc === '+0' ? 'muted' : 'strong'}">${row.inc}</span></td><td>${row.er}</td><td><div class="monitor-progress"><span class="cell-sub">${row.progress} ${row.pct}%</span><div class="progress-track"><div class="progress-fill" style="width:${row.pct}%"></div></div></div></td><td>${row.source}</td><td><span class="chip ${row.statusClass}">${row.status}</span></td><td>${index === 0 ? '<span class="row-hint">详情</span>' : ''}</td></tr>`).join('')}</tbody>
    </table></div>`
}
    ...
    <section id="content-list-section" class="content-list-anchor">
      ...
      <div class="list-filter-panel">
        <div class="list-filter-head">
          <div><strong>列表内筛选</strong><span>只影响下方监控内容列表，不改变数据看板、原始数据和贡献分析。</span></div>
        ...
        <div class="list-filter-row">
          <div class="search list-search">搜索频道 / 标题 / 链接</div>
          <span class="select">状态：全部</span>
          <span class="select">来源：全部</span>
          <span class="select">规则名：全部</span>
          <span class="select">数据问题：全部</span>
          <span class="select">短链状态：全部</span>
          <span class="select">预算状态：全部</span>
          <span class="select">评论状态：全部</span>
        </div>
  ...
  return `
    <section id="dashboard-section" class="dashboard-anchor">
      <div class="workbench-section-head dashboard-head">
        <div>
          <h2>数据看板</h2>
          <p>诊断观察区间内的内容变化与数据可信度，不做项目达标判断。</p>
        </div>
        ${observationControl()}
      </div>
      ${metricCards()}
      ...
      <div class="grid-2 mb-20">${efficiencyPanel()}${trustPanel(cross)}</div>
      ${contributionPanel()}
    </section>`
}
...
function projectRow(name, checked, views, er, issues, autoCount) {
  return `<div class="project-card-row ${checked ? 'selected' : ''}" data-project-card data-selected="${checked ? 'true' : 'false'}"><div><h2 class="project-title"><button class="row-check" data-row-check>${checked ? '☑' : '☐'}</button> ${name}</h2><div class="project-meta"><span class="avatar"></span><span>Yuki Chen</span><span class="divider"></span><span>协作者 4</span><span class="divider"></span><span>2026-04-18</span></div></div><div class="project-data"><div><strong>${views}</strong><div class="data-label">7天观看</div></div><div><strong>${er}</strong><div class="data-label">7天互动率</div></div><div><strong>${issues}</strong><div class="data-label">需关注</div></div><div><strong>${autoCount}</strong><div class="data-label">自动新增</div></div><div><strong>YT TT IG</strong><div class="data-label">平台</div></div></div><div class="project-actions"><span class="chip green">进行中</span></div></div>`
}
  ...
  return shell(`
    <div class="page">
      ...
      <section class="project-list-panel" data-project-list-panel>
        ...
        <div class="project-list-tools"><span class="select">卡片状态：进行中</span><span class="select">负责人：全部</span><span class="select">平台：全部</span><div class="search">搜索项目名称</div></div>
        ...
        <div class="project-list">${projectRow('May Creator Launch', false, '188.4K', '5.8%', 7, 13)}${projectRow('Beauty Review', false, '96.1K', '4.2%', 8, 4)}${projectRow('TikTok Spring Test', false, '73.6K', '6.7%', 6, 9)}${projectRow('Camera Trial', false, '29.8K', '2.1%', 1, 0)}${projectRow('Holiday Seeding', false, '12.6K', '2.9%', 3, 1)}</div>
        ${pagination()}
      </section>
    </div>`)
}
...
const listFilterLabels = {
  rank: '排序：观看增量降序',
  views: '排序：观看量降序',
  engagement: '排序：Engagement 增量降序',
  likes: '排序：点赞增量降序',
  comments: '排序：评论增量降序',
  shares: '排序：分享增量降序',
  er: '排序：ER 降序',
  'auto-track': '来源：自动追踪 · 监控创建时间：近期',
  ...
  document.querySelectorAll('[data-project-card]').forEach(card => {
    card.addEventListener('click', event => {
      ...
      if (document.querySelector('[data-project-list-panel]')?.classList.contains('is-batch')) {
        ...
        navigateTo('./option-a-project-workbench.html')
      }
    })
  })
