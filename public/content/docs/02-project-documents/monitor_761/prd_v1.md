...
- 不提升评论抓取能力。
- 不建设独立报告系统。
- 不承诺 PDF、图片等报告型导出；本期仅支持数据型 XLSX 导出。
- 不建设复杂异常检测算法。
- 不新增项目级 AI 总结。
- 不接入新平台或新内容形态。
- 不重构权限、配额、删除和导出体系。
...
| B12 | 通知系统是否支持自动追踪新增任务按用户每小时聚合发送 | 自动追踪通知 |
| B13 | 通知 deep link 是否支持进入工作台后自动打开任务详情抽屉 | 通知、详情抽屉 |
...
- 导出：实现全量导出、数据看板导出、当前视图导出三类 XLSX；每个文件至少包含 README sheet 和对应数据 sheet。
