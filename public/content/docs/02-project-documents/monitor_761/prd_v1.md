...
- 自动追踪新增任务可以通过通知系统按小时聚合提醒，且单条链接可进入对应项目并拉起任务详情抽屉。
...
- 单条内容行、Dashboard 内容榜单、通知单条任务链接都打开同一套单内容监控详情抽屉。
...
As an operator, I want automatic tracking additions to notify me in batches so that I can find new monitored tasks without checking manually all day.

Acceptance Criteria:

- 自动追踪规则成功新增监控任务后，生成通知候选事件。
- 通知按接收用户每小时汇总发送。
- 无新增任务时不发送空通知。
- 通知标题展示新增任务总数。
- 通知正文包含汇总时间窗口、涉及项目数、新增任务总数、项目分组、规则名和单条任务摘要。
- 点击单条任务链接进入对应项目工作台，并自动打开任务详情抽屉。
- 点击 `查看全部新增内容` 进入对应项目范围工作台，并应用 `近期新增=自动追踪入库` 的二级筛选。
- 已归档项目不产生自动追踪新增通知。
- 同一任务在同一批次内去重。
...
- 已归档项目不再通过自动追踪新增监控任务，也不产生自动追踪新增通知。
...
| B8 | 项目列表是否可返回最近 7 天速览指标 | 项目列表 |
| B9 | 跨项目视图是否支持项目 ID 多选过滤和区间增量 | 多项目工作台 |
| B10 | 现有全局列表控制台报错影响范围 | 旧入口迁移 |
| B11 | 通知系统是否支持自动追踪新增任务按用户每小时聚合发送 | 自动追踪通知 |
| B12 | 通知 deep link 是否支持进入工作台后自动打开任务详情抽屉 | 通知、详情抽屉 |
| B13 | 项目归档状态是否已有字段；若没有，需新增项目生命周期状态 | 项目列表、工作台 |
| B14 | 是否存在任务级 owner/responsible user 字段；若不存在，761 不实现任务归属人展示或批量修改 | 内容列表、详情抽屉、批量操作 |

### 4.7 Security & Privacy

- 所有项目、任务、通知 deep link 必须遵循当前账号权限和可见范围。
- 通知中不得发送当前用户不可见项目或任务。
...
| 项目速览聚合成本高 | 项目列表需要最近 7 天聚合指标 | 若接口暂不支持，先降级展示可得指标 |
...
| 通知频率过高 | 自动追踪新增可能很多 | 每小时汇总、任务去重、超量折叠 |
...
- Dashboard：当前只落地观察区间概览、内容变化榜、关键指标趋势、达人/频道贡献；效率指标观察、数据可信度、近期动向提醒不作为独立模块开发。
...
- 详情抽屉：统一承接行点击、榜单单条点击、通知单条任务链接；趋势图支持当前任务数据导出。
