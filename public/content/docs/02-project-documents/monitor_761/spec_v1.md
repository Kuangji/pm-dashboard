...
导出统一改为 `.xlsx`，用于承载多 sheet 数据。它是数据导出，不是报告生成；不提供 PDF、图片、富文本报告或自动解读。
...
| B12 | 通知系统是否支持自动追踪新增任务按用户每小时聚合发送 | M6 |
| B13 | 通知 deep link 是否支持进入工作台后自动打开任务详情抽屉 | M6/M3/M5 |
