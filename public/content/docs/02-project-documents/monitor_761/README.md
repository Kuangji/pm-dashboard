...
- 7611 补充口径：项目列表顶部改为“当前快照为主、处理提醒为辅”，工作台默认改为 `监控概览 / 数据看板` TAB 布局，归档支持撤销归档，自动追踪新增结果走全站公用系统通知；内容监控页面内不新增通知 UI。
...
- [ ] 7611 项目列表修正：顶部主要展示当前快照数据；“提醒周期”只影响需关注、自动追踪新增、短链待处理等处理提醒，不再定义当前总量。
