...
- **C 看板分析型**：不整体采用，保留其中“贡献分析”和“数据可信度”的表达方式，收敛到 A 的项目工作台内。
...
通知系统入口不新增独立页面。自动追踪抓取到新监控任务后，通知消息点击进入同一个 `内容监控工作台`，并根据任务链接自动拉出 `任务详情抽屉`。
...
| 场景 | 处理 |
| --- | --- |
| 项目运行中 | 默认展示在项目列表；允许进入工作台、添加监控、自动追踪新增和通知提醒 |
...
已归档项目仍可进入内容监控工作台。工作台顶部显示 `已归档` 标签，`添加监控` 禁用，自动追踪新增和自动追踪新增通知不再产生；数据看板、原始数据、监控内容列表、单内容抽屉和导出能力继续保留。
...
| 类型 | 控件位置 | 说明 |
| --- | --- | --- |
| 内容集合筛选 | 标题区下方 `content-filter-wrap` | 监控状态、地区、视频标签、额外标签1、额外标签2、负责人、创建人、监控时长、观看量、视频类型、来源/自动追踪规则、发布日期 |
...
- 效率指标：定位到列表，写入 `效率指标可计算` 或对应缺口条件。
- 数据可信度状态：定位到列表，写入评论、短链、预算、基线等问题条件。
...
效率指标不是本期 Dashboard 的核心变化入口，但需要给关心 CPM、CPV、CTR 等指标的用户一个稳定位置。为避免与 `原始数据` 中的 CPM/CPV/CTR 数值重复，Dashboard 里的效率指标观察只承担可计算性和缺口诊断，不重复展示具体结果值；具体数值统一在原始数据块中查看。
...
数据可信度作为 Dashboard 第三行的轻量模块，不做成大报告。
...
- 不放入状态、来源、规则名、标签、负责人、创建人、频道名/标题/链接搜索等内容集合筛选；这些维度保持在一级筛选中。
...
- 点击标签/状态/负责人等一级筛选维度：若需要按该维度重算全页数据，应引导用户使用一级筛选。
...
当用户从自动追踪新增通知点击单条任务进入时，工作台仍先按项目范围加载，随后自动打开任务详情抽屉。抽屉顶部增加轻量来源提示，说明该任务来自自动追踪通知、通知汇总批次和规则名；不改变当前列表筛选。
...
字段级编辑动作跟随字段出现，例如项目、负责人、监控时长、标签、短链和备注；不要在抽屉顶部集中堆叠编辑按钮。短链、评论情感、高频词模块即使暂无数据也保留模块位置，用空态解释当前不可用，而不是直接隐藏。
...
项目级筛选
  └─ 状态 / 地区 / 视频标签 / 额外标签1 / 额外标签2 / 负责人 / 创建人 / 监控时长 / 观看量 / 视频类型 / 来源 / 发布日期
...
自动追踪通知的单任务链接在 `projects` 之外追加任务定位参数：
...
- 先进入 `projects=<projectId>` 对应的单项目工作台。
- 页面加载完成后自动打开 `单内容监控` 抽屉。
- 抽屉顶部展示“来自自动追踪通知”提示，并显示规则名、通知批次时间和任务 ID。
- 该进入方式不自动改写 Dashboard、原始数据或内容列表筛选。
- 如果任务不可见或已失效，保留工作台页面，并在抽屉位置展示不可用提示。
...
- 与项目列表页项目列表筛选等价的筛选项：卡片状态、负责人、平台、项目名称搜索。
...
- 项目列表：[项目列表 Demo](../../../demos/monitor_761_layout_demos/project-list.html)
- 内容监控工作台（单项目范围入口）：[单项目工作台 Demo](../../../demos/monitor_761_layout_demos/option-a-project-workbench.html)
- 内容监控工作台（跨项目范围入口兼容页）：[跨项目工作台 Demo](../../../demos/monitor_761_layout_demos/option-d-ia-transition.html)

归档 Demo：

- B 列表运营型：[B 列表运营型 Demo](../../../demos/monitor_761_layout_demos/option-b-list-ops.html)
- C 看板分析型：[C 看板分析型 Demo](../../../demos/monitor_761_layout_demos/option-c-dashboard-board.html)
