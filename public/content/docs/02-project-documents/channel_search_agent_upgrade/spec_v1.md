...
| 纯 URL | 先识别 URL 类型 | ❌ 识别成功时不展开；⚠️ 无法识别或需修正时展开 | 走原有 URL 识别 / 精准搜索路径 |
...
3. `Soft Intent 提取`
   - 调性、性价比、近期表现、排除偏好等
4. `Conflict 检查`
   - 是否出现与 anchor 或 hard filters 冲突的描述
5. `Clarify or Search`
   - 能先搜则先搜，不能先搜再追问
