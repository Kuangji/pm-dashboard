...
- 受众画像筛选不进入本期正式范围；若设计稿中保留扩展位，也不得作为可用筛选项上线。
...
品牌数据在本版中保留结构位，但不是关键交付重点。

### 7.2 第一版规则

- 必须保留 `品牌数据` tab。
- 允许内容明显轻于其他分区。
- 若当前暂无足够稳定的品牌侧数据，可先以轻量占位方式保留结构。

### 7.3 品牌数据最小交付原则

- 若存在稳定可用的品牌相关数据，可先以最小列表或摘要模块承接。
- 若不存在，则以“结构保留 + 明确占位说明”处理。
- 第一版不得为了凑满四个 tab 而引入低可信度品牌推断结果。
